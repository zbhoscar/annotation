...
label_txt = r'D:\Dropbox\CodeZ\annotation_counting\list0915.txt'
...
# print([len(i) for i in list_real])

for xulie in train_list:
    name = '\\' + xulie[0].split('/')[0] + '\\' + xulie[0].split('/')[1] + '\\'
    orgpath = [ i for i in list_real if name in i ]
    if len(orgpath) != 0:
        src = orgpath[0]
        dst = new + src[len(org):]
